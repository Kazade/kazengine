...
bool frustum::is_cube_in_frustum(const float* p, const float size) const {
    float minX, maxX;
    float minY, maxY;
    float minZ, maxZ;
	...
	return is_aabb_in_frustum(minX, minY, minZ, maxX, maxY, maxZ);
}

bool frustum::is_aabb_in_frustum(float minX, float minY, float minZ,
...
bool frustum::is_aabb_in_frustum(const kmAABB& aabb) const {
